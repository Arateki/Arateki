...
### Open Graph image (og-image.png) — versioned, not built
The OG preview image (`apps/web/public/og-image.png`, 1200×630) is the social-share card referenced by `<meta property="og:image">`. It is **committed to the repo and is no longer regenerated by `pnpm build`** — the GitHub Actions self-hosted runner lacks the system libraries (`libatk-1.0.so.0`, GTK, etc.) that Playwright/Chromium needs to launch headless and render the SVG.

**Source of truth:** `apps/web/public/og-image.svg` — edit `<text>` elements, positions, colors, monogram, etc. directly in the SVG.

**Type rendering:** the `<text>`/`<tspan>` elements in the SVG declare `font-family="Helvetica, Arial, sans-serif"` only as a fallback. When `pnpm generate:og` runs, `apps/web/scripts/generate-og-image.mjs` reads the SVG via JSDOM, converts every `<text>` (and nested `<tspan>`) into `<path>` outlines using the Montserrat TTFs in `apps/web/src/assets/fonts/` (via `text-to-svg`), then renders the path-based SVG to PNG via Playwright. This means: (1) the PNG always uses real Montserrat regardless of which fonts the renderer's host has installed; (2) you do **not** need to edit paths by hand — keep editing `<text>` in the SVG and the script handles the outline conversion. Font weight maps: ≥700 → Bold, 500–699 → Medium, 400–499 → Regular, <400 → Light.

**When the source SVG changes, regenerate the PNG manually and commit it:**
```bash
pnpm --filter @arateki/web generate:og
git add apps/web/public/og-image.png
```

If you ever need to put OG generation back in CI, the runner needs `playwright install-deps chromium` (sudo required) — same prerequisite as the `pnpm prerender` step, which already runs Chromium for SSG.
